...
function parsable(node) {
  language(":root > *")(node);
}

function deparse_string(ast, with_whitespace, indent) {
  with_whitespace = with_whitespace === undefined ? true : with_whitespace;

  var whitespace = new WSManager(with_whitespace, indent || "  ");

  if (parseable(ast)) return;
...
function deparse_if(node) {
  ...
  var is_first_stmt = node.children[1].type === "stmt",
    ...
    is_second_stmt =
      has_second && node.children[2].children[0].type !== "stmtlist";
...
function deparse_stmt(node) {
  ...
  if (has_child && node.children[0].type === "decl") {
    if (
      node.children[0].children.length > 5 &&
      node.children[0].children[5].type === "function"
    ) {
